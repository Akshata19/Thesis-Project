...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule, MatButtonModule, RouterModule],
...
export class CategoriesComponent implements OnInit {
  ...
  addCategory(): void {
    ...
    this.http.post<any>('http://localhost:3000/api/categories', categoryData).subscribe(
      (response) => {
        if (response.success) {
